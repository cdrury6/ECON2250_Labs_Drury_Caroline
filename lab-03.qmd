...
2.  Now as a collaborators you can collaborate with your teammates repository, by cloning their repository. Go to their repository and copy the HTTPS url to clone their repository, this will looks like this:
    ...
    Then in RStudio, go to *File* $\rightarrow$ *New Pr
oject* $\rightarrow$ *Version Control* $\rightarrow$ *Git* and paste the Repository URL, and place it in a different folder than your original Rproject. Open the new R project in new window so that you keep your own project open.
...
Wendell is working with Caroline on this lab, he wrote this message :)
...
ggplot(lf_summary, aes(x = "", y = prop, fill = lfstatus)) +
  geom_bar(stat = "identity", width = 1, color = "white") +
  coord_polar("y") +
  geom_text(aes(label = label), 
            position = position_stack(vjust = 0.5), size = 4) +
  labs(title = "Labor Force Status Distribution", fill = "Status") +
  theme_void() +
...
ggplot(cps, aes(x = gender, y = earnwk, fill = gender)) +
  geom_violin(trim = FALSE) +
  geom_boxplot(width = 0.1, color = "black", alpha = 0.5) +
  labs(title = "Weekly Earnings by Gender (Violin Plot)") +
  theme_minimal()
```
